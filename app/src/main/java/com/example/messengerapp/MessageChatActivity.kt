...
class MessageChatActivity : AppCompatActivity() {
    ...
    private fun sendMessageToUser(senderId: String, receiverId: String?, message: String)
    {
        ...
        val messageHaspMap = HashMap<String, Any?>()
        messageHaspMap["sender"] = senderId
        messageHaspMap["message"] = message
        messageHaspMap["receiver"] = receiverId
        messageHaspMap["isseen"] = false
        messageHaspMap["url"] = ""
        messageHaspMap["messageId"] = messageKey
        reference.child("Chats")
            .child(messageKey!!)
            .setValue(messageHaspMap)
            ...
            //implemente the push notifications using fcm
        ...
        if (requestCode==438 && resultCode == RESULT_OK && data!=null && data!!.data!=null)
        {
            ...
            uploadTask.continueWithTask(Continuation <UploadTask.TaskSnapshot, Task<Uri>>{ task ->
                ...
                if(task.isSuccessful)
                {
                    ...
                    val messageHaspMap = HashMap<String, Any?>()
                    messageHaspMap["sender"] = firebaseUser!!.uid
                    messageHaspMap["message"] = "sent you an image."
                    messageHaspMap["receiver"] = userIdVisit
                    messageHaspMap["isseen"] = false
                    messageHaspMap["url"] = url
                    messageHaspMap["messageId"] = messageId

                    ref.child("Chats").child(messageId!!).setValue(messageHaspMap)
                        .addOnCompleteListener { task ->
                            if (task.isSuccessful)
                            {
                                progressBar.dismiss()
